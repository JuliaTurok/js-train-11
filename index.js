...
function setToArray(mySet) {
  ...
  const arraySet = [...mySet];
  const newArray = arraySet.filter((element) => typeof element === `string`);
  ...
  const uniqueSet = new Set();
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (uniqueSet.has(currentElement)) {
            ...
            uniqueSet.delete(currentElement);
            i--;
        }
        else{
...
function iterateSet(set) {
  ...
  const recordIterator = set.entries();
  for(const entry of recordIterator){
    console.log(entry)
  }
}
